const books = [
  ...
  function rendersavedbooks() {
    ...
    savedcontainer.innerHTML = ""; // clear previous render

    if (savedbooks.length === 0) {
      savedcontainer.innerHTML = `<p class="empty-msg">No saved books yet!</p>`;
    } else {
      savedbooks.forEach(bookId => {
        const bookitem = books.find(b => b.id === bookId);
        if (!bookitem) return; // Skip if book not found

        const div = document.createElement("div");
        div.classList.add("book-card");

        div.innerHTML = `
        <img src="${bookitem.image}" alt="${bookitem.title} cover" class="book-img" />
        <div class="book-info">
          <h3 class="book-title">${bookitem.title}</h3>
          <p class="book-author">${bookitem.author}</p>
          <p class="book-genres">${bookitem.genres.join(", ")}</p>
          <p class="book-price">$${bookitem.priceUsd.toFixed(2)}</p>
          <button class="btn-remove" data-book-id="${bookitem.id}">Remove</button>
        </div>
      `;

        // Attach event listener instead of inline onclick
        div.querySelector(".btn-remove").addEventListener("click", () => remove(bookitem.id));
    ...
    // Update saved books quantity
    const len = savedbooks.length;
    document.getElementById("saved-quantity").textContent = len;
    document.getElementById("h1count").textContent = `SavedBooks (${len})`;
  }
  ...
  let len = savedbook.length;

  document.getElementById("saved-quantity").textContent = `${len}`;

  function clearall() {
    let savedbooks = JSON.parse(localStorage.getItem("savedboks"));
    savedbooks = [];
    localStorage.setItem("savedbooks", JSON.stringify(savedbooks));
    rendersavedbooks();
  }
  ...
  function searchfun() {
    ...
    let savedbooks = JSON.parse(localStorage.getItem("savedbooks")) || [];
    const savedBookObjs = books.filter(b => savedbooks.includes(b.id));
    const filtered = savedBookObjs.filter(book =>
      book.title.toLowerCase().includes(searchval) ||
      (book.author && book.author.toLowerCase().includes(searchval))
    );
    ...
    if (filtered.length === 0) {
      ...
      filtered.forEach(bookitem => {
        const div = document.createElement("div");
        div.classList.add("book-card");
        div.innerHTML = `
            <img src="${bookitem.image}" alt="${bookitem.title} cover" class="book-img" />
            <button class="btn-remove" type="button" data-book-id="${bookitem.id}" onclick="remove(${bookitem.id})">Remove</button>
        `;
        savedcontainer.appendChild(div);
      });
    }
    document.getElementById("saved-quantity").textContent = `${filtered.length}`;
  }
  ...
  function genersfun() {
    ...
    savedbooks.forEach(book => {
      const bookitem = books.find(b => b.id === book);
      if (
        bookitem && Array.isArray(bookitem.genres) &&
        bookitem.genres.some(
