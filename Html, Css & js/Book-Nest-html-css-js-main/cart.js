const books = [
  ...
  function updateQuantity(bookId) {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    let item = cart.find(b => b.bookId === bookId);
  ...
  function updateQuantity1(bookId) {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    let item = cart.find(b => b.bookId === bookId);
  ...
  function rendercart() {
    ...
    cart.forEach((item, index) => {
      ...
      if (book) {
        totalPrice += book.priceUsd * item.quantity;
        ...
        box.innerHTML = `
                  ...
                  <div class="details">
                       ...
                       <p><strong>Price:</strong> $${(book.priceUsd * item.quantity).toFixed(2)} (${book.priceUsd}$ per each item)</p>
                       <div class="quantity-control">
                           <button class="qty-btn" onclick="updateQuantity1(${book.id})">−</button>
                           <span class="qty-value" id="qty-${book.id}">${item.quantity}</span>
                           <button class="qty-btn" onclick="updateQuantity(${book.id})">+</button>
                       </div>

                       <div class="details-actions">
                      <button class="btn-secondary remove" onclick="removebook(${book.id})">Remove</button>
                     <a href="book.html"> <button class="btn-secondary viewmore" onclick="viewmore(${book.id})">View more</button></a>
                       </div>
                  </div>
    ...
    if (cart.length > 0) {
      ...
      shippingprice.textContent = (totalPrice * 0.15).toFixed(2);
      taxprice.textContent = ((totalPrice * 0.15).toFixed(2) * 0.25).toFixed(2);
      grandtotal.textContent = (((totalPrice) + ((totalPrice * 0.15)) + (((totalPrice * 0.15) * 0.25)))).toFixed(2);
    } else {
