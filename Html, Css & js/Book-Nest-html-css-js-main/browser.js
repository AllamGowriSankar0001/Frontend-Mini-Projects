...
<div class="details">
    ...
    <div class="details-actions">
        ...
        <a href="book.html">
        <button class="btn-secondary btn-view-more" type="button" onclick="viewmore(${book.id})">View more</button></a>
    </div>
</div>
`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedCart) {
        ...
        // Calculate cart quantity from cart items
        let cartquantity = 0;
        cart.forEach(item => {
...
function ratinghigh() {
    ...
    if (sortvalue.value == "rating-high") {
        books.sort((x, y) => y.rating - x.rating);
        renderBooks();
    } else if (sortvalue.value == "rating-low") {
        books.sort((x, y) => x.rating - y.rating);
        renderBooks()
    } else if (sortvalue.value == "price-high") {
        books.sort((x, y) => y.priceUsd - x.priceUsd);
        renderBooks();
    } else if (sortvalue.value == "price-low") {
        books.sort((x, y) => x.priceUsd - y.priceUsd);
        renderBooks();
    }
}
